...
double Adjustthreshold::getAdjustThreshold(cv::Mat src_Mat, int whichThresholdMethod, double thresholdError)
{
    // calculate Mean
    cv::Scalar tmp_mean;
...
cv::Mat Adjustthreshold::getOrigMat()
{
    if(imgExist)
        return origImg;
}

cv::Mat Adjustthreshold::getGrayMat()
{
    if(imgExist)
        return grayImg;
}
