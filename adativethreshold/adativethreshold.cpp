#include "adativethreshold.h"

Adativethreshold::Adativethreshold()
{
}

bool Adativethreshold::readImg(std::string imgName)
{
...
double Adativethreshold::getGlobalAdativeThreshold(int whichThresholdMethod, double thresholdError)
{
    if(imgExist)
    {
        // calculate Mean
...
double Adativethreshold::getGlobalAdativeThreshold(cv::Mat src_Mat, int whichThresholdMethod, double thresholdError)
{
    // calculate Mean
    cv::Scalar tmp_mean;
    ...
    while(!done)
    {
        ...
        // calculate two group' mean & std except 0
        cv::meanStdDev(src_Mat, tmp_mean1, tmp_stddev1, src_Mat > threshold);
        cv::meanStdDev(src_Mat, tmp_mean2, tmp_stddev2, src_Mat < threshold);
        ...
        {
            cv::Mat tmp_Mat;
            threshold = cv::threshold(src_Mat, tmp_Mat, 0, 255, cv::THRESH_OTSU);
            break;
        }
        done = (abs(threshold - thresholdNext) < thresholdError);
        threshold = thresholdNext;
...
cv::Mat Adativethreshold::kcircle(int kCircleRadius)
{
...
cv::Mat Adativethreshold::morphologyClosingOpening(cv::Mat src_Mat, int kCircleRadius)
{
...
cv::Mat Adativethreshold::getOrigMat()
{
    if(imgExist)
        return origImg;
}

cv::Mat Adativethreshold::getGrayMat()
{
    if(imgExist)
        return grayImg;
}
